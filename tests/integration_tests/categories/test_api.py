...
async def test_post_category(ac, user, title, status_code):
    response = await ac.post("/categories/", json={"title": title, "user_id": user["id"]})
...
async def test_get_categories_by_user(ac, user):
    ...
    response = await ac.get("/categories/", params={"user_id": user["id"], "page": 1, "size": 10})
...
async def test_update_category(ac, category):
    response = await ac.patch(
        "/categories/", params={"category_id": category["id"]}, json={"title": "Updated Food"}
    )
...
async def test_update_nonexistent_category(ac):
    response = await ac.patch("/categories/", params={"category_id": 99999}, json={"title": "Nope"})

    assert response.status_code == 404


async def test_delete_category(ac, category):
    response = await ac.delete("/categories/", params={"category_id": category["id"]})
