...
from src.db.crud.expenses import (
    get_expense_db,
    get_expenses_by_user_db,
    add_expense_db,
    update_expense_db,
    delete_expense_db,
)
...
expenses_router = APIRouter(
    prefix="/expenses", dependencies=[Depends(verify_bot_api_key)], tags=["Expenses"]
)


@expenses_router.get("/{expense_id}", response_model=ExpenseResponse)
async def get_expense(expense_id: int, db: DBDep):
...
async def get_expenses(
    ...
    return await get_expenses_by_user_db(
...
async def create_expense(expense_data: ExpenseCreate, db: DBDep):
    response = await add_expense_db(expense_data, db)
    await redis_manager.scan_delete(
        pattern=ExpensesCacheKeyBuilder.generate_pattern(response.user_id)
    )
    return response


@expenses_router.patch("/", response_model=ExpenseResponse)
async def update_expense(expense_id: int, update_data: ExpenseUpdate, db: DBDep):
